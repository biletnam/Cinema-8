...
create table sale (
    nr_sali TINYINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
create table projekcje (
    ...
    CONSTRAINT projekcje_fk_sala FOREIGN KEY(nr_sali) REFERENCES sale(nr_sali) on DELETE CASCADE on UPDATE CASCADE, 
    CONSTRAINT projekcje_fk_film FOREIGN KEY(id_filmu) REFERENCES filmy(id_filmu) on DELETE CASCADE on update CASCADE,
...
create table rezerwacje (
    ...
    CONSTRAINT rezerwacje_unique UNIQUE(nr_miejsca, id_klienta, id_projekcji)
);
...
#triger przy insercie do sprawdzania czy w danym dniu o danej godzinie w danej sali może być projekcja
